...
plot_cis_avg2v <- ggplot(data = all_results_cis_avgByPatient_slice, aes(y = avg_minus_log_p_fdr, x = avg_neg_zscore_minus_log2_int_freq_tolerance, color = positive_outlier_and_significant), na.rm = T, se = TRUE) +
  geom_point(size = 3, alpha = .5) +
  scale_color_manual(values = c("gray", "violet")) +
  scale_size(name = "Stdev P-value") + 
  facet_grid( Study ~ .) +
  ...
  theme(strip.text.y = element_text(size = 16, colour = "blue"), strip.text.x = element_text(size = 16, colour = "blue")) +
  theme(strip.text = element_text(face="bold", size=16)) +
...
png(file = paste(cis_base_folder, analysis_folder_date, "/", analysis_folder_date, ".CIS.results_invivo.avg_by_trial.figure2v.png", sep = ""), height=12, width=6, units = "in", res = 300)
print(plot_cis_avg2v)
dev.off()
pdf(file = paste(cis_base_folder, analysis_folder_date, "/", analysis_folder_date, ".CIS.results_invivo.avg_by_trial.figure2v.pdf", sep = ""), height=12, width=6)
print(plot_cis_avg2v)
dev.off()
...
#### ------- After covariate regression and fitting ----- Buscaroli Caravagna 07/12/2022
# data and results processed by Python Skit, available in the folder
# 1. load data
# patient_h_scaled_summary_filtered_fit <- read.csv(file = "/Users/calabria.andrea/Dropbox (HSR Global)/TIGET/sharing/Giulio Caravagna/HSC dynamics/regression/202112.Diversity.ByMarker.Hindex.csv", header=T, fill=T, check.names = FALSE, sep = ',', na.strings = c("NA", ''), row.names = 1)
patient_h_scaled_summary_filtered_fit <- read.csv(file = "/Users/calabria.andrea/Dropbox (HSR Global)/TIGET/sharing/Giulio Caravagna/HSC dynamics/diversity/datasets/PRED_202112.Diversity.ByMarker.Hindex.csv", header=T, fill=T, check.names = FALSE, sep = ',', na.strings = c("NA", ''), row.names = 1)
# select the most stringent formula including all covariates
patient_h_scaled_summary_filtered_fit3 <- patient_h_scaled_summary_filtered_fit[which(patient_h_scaled_summary_filtered_fit$fitname == "fit2"),]

patient_h_scaled_summary_filtered_fit3$Study <- factor(patient_h_scaled_summary_filtered_fit3$ClinicalStudy, levels = study_list)
patient_h_scaled_summary_filtered_fit3$TimePoint <- patient_h_scaled_summary_filtered_fit3$Timepoint
patient_h_scaled_summary_filtered_fit3[is.na(patient_h_scaled_summary_filtered_fit3)] <- NA

blood_lineages <- read.xlsx(xlsxFile = blood_lineages_file, sheet = "blood_lineages")
blood_lineages_colors <- read.xlsx(xlsxFile = blood_lineages_file, sheet = "colors")
blood_lineages <- merge(x = blood_lineages, y = blood_lineages_colors, all.x = T, by = c("CellType"))
rownames(blood_lineages) <- blood_lineages$CellMarker
# patient_h_scaled_summary_filtered_fit3 <- merge(x = patient_h_scaled_summary_filtered_fit3, y = blood_lineages, by = c("CellMarker"), all.x = T)

patient_h_scaled_summary_filtered_fit3$CellLineage <- factor(patient_h_scaled_summary_filtered_fit3$CellType, levels = celltype_list)
scale_color_manual_colors_sortedbyname <- blood_lineages_colors[order(blood_lineages_colors$CellType), "colorcode"]

# 2. plot data (as usual)
plot_hindex_homogeneouspcr_fit3 <-
  ggplot(data = patient_h_scaled_summary_filtered_fit3, aes(x = TimePoint, y = pred_Hindex, fill = CellType, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  geom_smooth(method = "loess", formula = y ~ splines::ns(x), stat = "smooth", position = "identity", alpha = 0.6, level = 0.75) +
  scale_x_continuous(breaks = seq(0, max(patient_h_scaled_summary_filtered_fit3$TimePoint, na.rm = T), 12)) +
  facet_grid( Tissue ~ Study, scales = "free_x", space = "free") +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
  theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") + 
  theme(axis.text.x = element_text(size=16), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  # theme(strip.text.y = element_text(size = 16, colour = "blue", angle = 0)) +
  labs(title = paste0("Diversity index (Shannon)"), subtitle = paste0("Patients with homogeneous PCR methods over time (no mixed patients).\nMulti-regression model with covariates: ", unique(patient_h_scaled_summary_filtered_fit3$regression_formula), "."),
       x = "Months after gene therapy", 
       y = "Shannon diversity index (H') with covariates", 
       colour = "CellMarker", fill = "CellMarker")

pdf(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.pdf", sep = ""), height=8, width=12)
plot(plot_hindex_homogeneouspcr_fit3)
dev.off()
png(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.png", sep = ""), height=8, width=12, units = "in", res = 300)
plot(plot_hindex_homogeneouspcr_fit3)
dev.off()

# # 3. compare data (now importing z-scaled fitted values)
patient_h_scaled_summary_filtered_fit3 <- patient_h_scaled_summary_filtered_fit3 %>% group_by(SubjectID, Tissue, TimePoint) %>% mutate(pred_h_zscaled=scale(pred_Hindex))
patient_h_scaled_summary_filtered_fit3$TimepointMonths <- patient_h_scaled_summary_filtered_fit3$TimePoint

patient_h_scaled_summary_filtered_fit3_stable <- patient_h_scaled_summary_filtered_fit3[which(patient_h_scaled_summary_filtered_fit3$TimePoint >= 24 & patient_h_scaled_summary_filtered_fit3$TimePoint <= 62),]
patient_h_scaled_summary_filtered_fit3_stable <- patient_h_scaled_summary_filtered_fit3_stable %>% group_by(SubjectID, Tissue, TimePoint) %>% mutate(n_obs_stt_overtime = n())
patient_h_scaled_summary_filtered_fit3_stable <- patient_h_scaled_summary_filtered_fit3_stable %>% group_by(SubjectID, Tissue, TimePoint) %>% mutate(n_obs_stt_overtime = n()) %>% filter(n_obs_stt_overtime >= 3)

# summary stats
patient_h_scaled_fit3_stable_summary <- patient_h_scaled_summary_filtered_fit3_stable %>%
  filter(n_obs_stt_overtime >= 3) %>%
  group_by(Study, SubjectID, Tissue, CellMarker, CellType) %>%
  # get_summary_stats(PercNIS_onOverallSharedCD34BM_zscaled, type = "mean_sd")
  get_summary_stats(pred_h_zscaled, type = "full")

patient_h_scaled_summary_filtered <- patient_h_scaled_fit3_stable_summary %>% filter(n >= 2)
patient_h_scaled_summary_filtered$CellLineage <- factor(patient_h_scaled_summary_filtered$CellType, levels = celltype_list)
patient_h_scaled_summary_filtered$Study <- factor(patient_h_scaled_summary_filtered$Study, levels = study_list)
# patient_h_scaled_summary_filtered$AgeGroup_Sorted <- factor(patient_h_scaled_summary_filtered$AgeGroup, levels = age_list)
...
# plot data (as usual)
plot_hindex_homogeneouspcr_fit3_z <-
  ggplot(data = patient_h_scaled_summary_filtered_fit3, aes(x = TimePoint, y = pred_h_zscaled, fill = CellType, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  geom_smooth(method = "loess", formula = y ~ splines::ns(x), stat = "smooth", position = "identity", alpha = 0.6, level = 0.75) +
  scale_x_continuous(breaks = seq(0, max(patient_h_scaled_summary_filtered_fit3$TimePoint, na.rm = T), 12)) +
  ...
  theme(axis.text.x = element_text(size=16), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  # theme(strip.text.y = element_text(size = 16, colour = "blue", angle = 0)) +
  labs(title = paste0("Diversity index (Shannon)"), subtitle = paste0("Patients with homogeneous PCR methods over time (no mixed patients). Z-score normalization by Patient, Tissue, Timepoint.\nMulti-regression model with covariates: ", unique(patient_h_scaled_summary_filtered_fit3$regression_formula), "."),
       x = "Months after gene therapy", 
       y = "Shannon diversity index (H') with covariates", 
       colour = "CellMarker", fill = "CellMarker")

pdf(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.pdf", sep = ""), height=8, width=12)
plot(plot_hindex_homogeneouspcr_fit3_z)
dev.off()
png(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.png", sep = ""), height=8, width=12, units = "in", res = 300)
plot(plot_hindex_homogeneouspcr_fit3_z)
dev.off()
...
p_h_scaled_summary_stable_min3_stats <- ggplot(data = patient_h_scaled_summary_filtered,
  ...
  theme(axis.text.x = element_text(size=16), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  labs(title = paste0("Clonal Diversity"), 
       subtitle = paste0("Filters: samples with homogeneous PCR technology within the same trial, PB and BM separated, markers separated. Months under analysis: 24-60.\nMulti-regression model with covariates: ", unique(patient_h_scaled_summary_filtered_fit3$regression_formula), ". Normalization: Z-score by patient, tissue, time point. "),
       x = "Studies", 
       y = "Shannon diversity index (Z-score)", 
       colour = "CellMarker", fill = "CellMarker")

pdf(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_across_trials.pdf", sep = ""), height=8, width=12)
plot(p_h_scaled_summary_stable_min3_stats)
dev.off()
png(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_across_trials.png", sep = ""), height=8, width=12, units = "in", res = 300)
plot(p_h_scaled_summary_stable_min3_stats)
dev.off()
...
pdf(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_across_trials.colored.pdf", sep = ""), height=8, width=12)
plot(p_h_scaled_summary_stable_min3_stats_newcolors)
dev.off()
png(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_across_trials.colored.png", sep = ""), height=8, width=12, units = "in", res = 300)
plot(p_h_scaled_summary_stable_min3_stats_newcolors)
dev.off()
...
pdf(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_within_trials.pdf", sep = ""), height=8, width=12)
plot(p_h_scaled_summary_stable_min3_stats_lineage)
dev.off()
png(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_within_trials.png", sep = ""), height=8, width=12, units = "in", res = 300)
plot(p_h_scaled_summary_stable_min3_stats_lineage)
dev.off()

p_h_scaled_summary_stable_min3_stats_lineage_newcolors <- 
  ggplot(data = patient_h_scaled_summary_filtered,
         aes(x = CellLineage, y = mean, colour = CellType, fill = CellType, group = CellLineage), na.rm = T, se = TRUE) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  ...
  stat_compare_means(comparisons = my_comparisons, label.y = seq(from = perc_starting_max_stats, to = (perc_starting_max_stats+(length(my_comparisons)*perc_spacer_stats)), by = perc_spacer_stats)*10^-1, p.adjust.method = "fdr", label = "p.signif") +
  theme(strip.text = element_text(face="bold", size=16)) +
  theme(axis.text.x = element_text(size=16, angle = 30, vjust = 1, hjust=1), 
...
pdf(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_within_trials.colors.pdf", sep = ""), height=8, width=12)
plot(p_h_scaled_summary_stable_min3_stats_lineage_newcolors)
dev.off()
png(file = paste(h_base_folder, analysis_folder_date, "/PaperRev1.", analysis_folder_date, ".Diversity.ByMarker.HomoPCR.fit3.Zscore.statistics_within_trials.colors.png", sep = ""), height=8, width=12, units = "in", res = 300)
plot(p_h_scaled_summary_stable_min3_stats_lineage_newcolors)
dev.off()
...
##### ==========================
#### applying the Good Turing model and correcting by confounding factors by Buscaroli and Caravagna (2023-10-10):
followup_limit_crosstrial <- 60
patients_summary <- read.xlsx(xlsxFile = "source/Clinical/Patient Summary.xlsx", sheet = "summary")
allpatients_shared34_stats <- read.csv(file = paste("/Users/calabria.andrea/Dropbox (HSR Global)/TIGET/sharing/Giulio Caravagna/HSC dynamics/cd34output/datasets/PRED_gtratio_cd34shared.csv", sep = ""), header=TRUE, fill=T, sep=',', check.names = FALSE, na.strings = c("NA", ''))
allpatients_shared34_stats$PercNIS_onOverallSharedCD34BM <- allpatients_shared34_stats$pred
allpatients_shared34_stats$TimepointMonths <- allpatients_shared34_stats$Timepoint
allpatients_shared34_stats$FollowUp <- allpatients_shared34_stats$Timepoint
allpatients_shared34_stats$PercNIS_onOverallSharedCD34BM <- ifelse(allpatients_shared34_stats$PercNIS_onOverallSharedCD34BM < 0, allpatients_shared34_stats$PercNIS_onOverallSharedCD34BM == 0, allpatients_shared34_stats$PercNIS_onOverallSharedCD34BM)
# isolate SCID patients
scidpatients_shared34_stats <- allpatients_shared34_stats[which(allpatients_shared34_stats$ClinicalStudy == "XSCID"),]
# our patients now
allpatients_shared34_stats <- merge(x = allpatients_shared34_stats, y = patients_summary, by = c("SubjectID"), all.x = T)
# allpatients_shared34_stats$AgeGroup 
allpatients_shared34_stats <- allpatients_shared34_stats %>%
...
# select only the largest fit, fit6
# allpatients_shared34_stats_slice <- allpatients_shared34_stats[which(allpatients_shared34_stats$fitname == "fit2" & allpatients_shared34_stats$fittype == "ageGroup"),] 
allpatients_shared34_stats_slice <- allpatients_shared34_stats[which(allpatients_shared34_stats$fitname == "fit1" & allpatients_shared34_stats$fittype == "ageGroup"),] 

allpatients_shared34_stats_noCD34 <- allpatients_shared34_stats_slice[which(!(allpatients_shared34_stats_slice$CellMarker %in% c("CD34", "Plasma")) & 
                                                                              !(is.na(allpatients_shared34_stats_slice$colorcode)) ),]
# allpatients_shared34_stats_noCD34$PercNIS_onOverallSharedCD34BM <- (allpatients_shared34_stats_noCD34$SharedCD34IS_NumIS * 100) / allpatients_shared34_stats_noCD34$Progenitor_nIS
allpatients_shared34_stats_noCD34$Study <- factor(allpatients_shared34_stats_noCD34$ClinicalStudy, levels = study_list)
...
allpatients_shared34_stats_noCD34 <- allpatients_shared34_stats_noCD34 %>% group_by(SubjectID, Tissue, TimepointMonths) %>% mutate(PercNIS_onOverallSharedCD34BM_zscaled=scale(PercNIS_onOverallSharedCD34BM))
allpatients_shared34_stats_noCD34[is.na(allpatients_shared34_stats_noCD34)] <- NA
# apply filters by zscore, where missing remove it from plot
allpatients_shared34_stats_noCD34 <- allpatients_shared34_stats_noCD34[which(!is.na(allpatients_shared34_stats_noCD34$PercNIS_onOverallSharedCD34BM_zscaled)),]
allpatients_shared34_stats_supergroup_noCD34 <- allpatients_shared34_stats_noCD34

# repeat zscale for SCID patients
scidpatients_shared34_stats <- scidpatients_shared34_stats %>% group_by(SubjectID, Tissue, TimepointMonths) %>% mutate(PercNIS_onOverallSharedCD34BM_zscaled=scale(PercNIS_onOverallSharedCD34BM))
scidpatients_shared34_stats[is.na(scidpatients_shared34_stats)] <- NA
# apply filters by zscore, where missing remove it from plot
scidpatients_shared34_stats <- scidpatients_shared34_stats[which(!is.na(scidpatients_shared34_stats$PercNIS_onOverallSharedCD34BM_zscaled)),]

shared_cols <- intersect(colnames(allpatients_shared34_stats_noCD34), colnames(scidpatients_shared34_stats))
all4studies_shared34_stats_noCD34 <- rbind(allpatients_shared34_stats_noCD34[shared_cols], scidpatients_shared34_stats[shared_cols])
all4studies_shared34_stats_noCD34$Study <- factor(all4studies_shared34_stats_noCD34$ClinicalStudy, levels = c(study_list, "XSCID"))

scale_color_manual_colors_sortedbyname <- colors_lineages[1,]
p_lines_mbte_legendbottom <- 
  ggplot(data = allpatients_shared34_stats_noCD34, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM, color = CellType), na.rm = T, se = TRUE) +
  theme_bw() +
  # ggplot(data = allpatients_shared34_stats_noCD34[which(!(allpatients_shared34_stats_noCD34$SubjectID %in% c(patients_to_exclude, patients_without_erythroid, patients_uncertainty))),], aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  # geom_point(aes(shape = CellMarker), size=4, alpha = .6) +
  stat_smooth(aes(fill = CellType), formula = y ~ log(x), level = 0.4, size = 2) +
  ...
  theme(strip.text.x = element_text(size = 16, colour = "darkblue", angle = 0, face="bold"), 
        strip.text.y = element_text(size = 16, colour = "darkred", angle = 270, face="bold")) +
...
p_scid_lines_mbte_identicalFU <- 
  ggplot(data = scidpatients_shared34_stats, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  stat_smooth(aes(fill = CellType), formula = y ~ log(x), level = 0.4, size = 2) +
  facet_grid(. ~ ClinicalStudy, scales = "free_x", space = "free_x") +
  # scale_y_continuous(labels = scales::percent, limits = c(0, max(allpatients_shared34_stats_noCD34$PercNIS_onOverallSharedCD34BM))) +
...
p_4trials_lines_mbte_identicalFU <- 
  ggplot(data = all4studies_shared34_stats_noCD34, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  stat_smooth(aes(fill = CellType), formula = y ~ log(x), level = 0.4, size = 2) +
  # facet_grid(. ~ ClinicalStudy, scales = "free_x", space = "free_x") +
...
p_lines_mbte_zscore <- 
  ggplot(data = allpatients_shared34_stats_noCD34, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM_zscaled, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  stat_smooth(aes(fill = CellType), formula = y ~ log(x), level = 0.4, size = 2) +
  facet_grid(. ~ Study, scales = "free_x", space = "free_x") +
...
p_4studies_lines_mbte_zscore <- 
  ggplot(data = all4studies_shared34_stats_noCD34, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM_zscaled, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  stat_smooth(aes(fill = CellType), formula = y ~ log(x), level = 0.4, size = 2) +
  facet_grid(. ~ Study, scales = "free_x", space = "free_x") +
...
p_lines_mbte_agegr_nopoints <- 
  ggplot(data = allpatients_shared34_stats_supergroup_noCD34, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM, color = CellType), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  stat_smooth(aes(fill = CellType), formula = y ~ log(x), level = 0.4, size = 2) +
  facet_grid(AgeGroup_Sorted ~ Study, scales = "free_x", space = "free_x") +
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.pdf", sep = ""), 
    # height=4.5, width=10)
    height=5.5, width=10)
print(p_lines_mbte_legendbottom)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.png", sep = ""), 
    # height=4.5, width=10, units = "in", res = 300)
    height=5.5, width=10, units = "in", res = 300)
print(p_lines_mbte_legendbottom)
dev.off()

pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), "_XSCID.CD34output.allpatients.wrap_trials.pdf", sep = ""), 
    # height=4.5, width=10)
    height=10, width=8)
print(p_4trials_lines_mbte_identicalFU)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), "_XSCID.CD34output.allpatients.wrap_trials.png", sep = ""), 
    # height=4.5, width=10, units = "in", res = 300)
    height=8, width=8, units = "in", res = 300)
print(p_4trials_lines_mbte_identicalFU)
dev.off()

pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.SCID.CD34output.allpatients.wrap_trials.pdf", sep = ""), 
    # height=4.5, width=10)
    height=5, width=5.5)
print(p_scid_lines_mbte_identicalFU)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.SCID.CD34output.allpatients.wrap_trials.png", sep = ""), 
    # height=4.5, width=10, units = "in", res = 300)
    height=5, width=5.5, units = "in", res = 300)
print(p_scid_lines_mbte_identicalFU)
dev.off()

pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.pdf", sep = ""), height=5, width=10)
print(p_lines_mbte_zscore)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.png", sep = ""), height=5, width=10, units = "in", res = 300)
print(p_lines_mbte_zscore)
dev.off()

pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.zscoe.wrap_trials_age.pdf", sep = ""), 
    height=7.5, width=10)
print(p_lines_mbte_zscore_agegr)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.zscore.wrap_trials_age.png", sep = ""), 
    height=7.5, width=10, units = "in", res = 300)
print(p_lines_mbte_zscore_agegr)
dev.off()

pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials_age.nopoints.pdf", sep = ""), 
    height=9, width=10)
print(p_lines_mbte_agegr_nopoints)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials_age.nopoints.png", sep = ""), 
    height=9, width=10, units = "in", res = 300)
print(p_lines_mbte_agegr_nopoints)
dev.off()
...
p_lines_mbte_trial <- ggplot(data = allpatients_shared34_stats_supergroup_noCD34, aes(x = FollowUp, y = PercNIS_onOverallSharedCD34BM, color = Study), na.rm = T, se = TRUE) +
  ...
  theme_bw() +
  theme(strip.text.x = element_text(size = 16, colour = "darkblue", angle = 0, face="bold"), strip.text.y = element_text(size = 16, colour = "darkred", angle = 270, face="bold")) +
  theme(axis.text.x = element_text(size=16, angle = 0), axis.text.y = element_text(size=16), axis.title = element_text(size=18), plot.title = element_text(size=20)) +
  labs(title = paste("Profile of CD34 BM output towards Myeloid/B/T/Erythroid lineages"), x = "FollowUp months after GT", y = "Perc. of IS shared with CD34 BM", 
       subtitle = "All clinical trials included, all patients included. WAS clinical trial includes early progenitors as supergroup of CD34.")


pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.byLineage.pdf", sep = ""), height=5, width=17)
print(p_lines_mbte_trial)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.byLineage.png", sep = ""), height=5, width=17, units = "in", res = 300)
print(p_lines_mbte_trial)
dev.off()
...
# include only here SCID
all4studies_shared34_stats_supergroup_noCD34_stable <- all4studies_shared34_stats_noCD34[which(all4studies_shared34_stats_noCD34$TimepointMonths >= 24 & all4studies_shared34_stats_noCD34$TimepointMonths <= 60),]
all4studies_shared34_stats_supergroup_noCD34_stable <- all4studies_shared34_stats_supergroup_noCD34_stable %>% group_by(SubjectID, Tissue, TimepointMonths) %>% mutate(n_obs_stt_overtime = n())
all4studies_shared34_stats_supergroup_noCD34_stable_min3 <- all4studies_shared34_stats_supergroup_noCD34_stable %>% group_by(SubjectID, Tissue, TimepointMonths) %>% mutate(n_obs_stt_overtime = n()) %>% filter(n_obs_stt_overtime >= 3)
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg.pdf", sep = ""), height=5, width=12)
plot(p_lines_mbte_zscore_stable_min3)
dev.off()
...
write.xlsx(x = patient_shared34_summary_filtered, file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg.patient_shared34_summary_filtered.full.xlsx", sep = ""))

# summary stats
patient_withSCID_shared34_summary <- all4studies_shared34_stats_supergroup_noCD34_stable_min3 %>%
  filter(n_obs_stt_overtime >= 3) %>%
  group_by(Study, SubjectID, Tissue, CellMarker, CellType) %>%
  # get_summary_stats(PercNIS_onOverallSharedCD34BM_zscaled, type = "mean_sd")
  get_summary_stats(PercNIS_onOverallSharedCD34BM_zscaled, type = "full")

patient_withSCID_shared34_summary_filtered <- patient_withSCID_shared34_summary %>% filter(n >= 2)
patient_withSCID_shared34_summary_filtered$CellLineage <- factor(patient_withSCID_shared34_summary_filtered$CellType, levels = celltype_list)
patient_withSCID_shared34_summary_filtered$Study <- factor(patient_withSCID_shared34_summary_filtered$Study, levels = c(study_list, "XSCID"))
# patient_withSCID_shared34_summary_filtered$AgeGroup_Sorted <- factor(patient_shared34_summary_filtered$AgeGroup, levels = age_list)
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg.byStudy.pdf", sep = ""), height=4.5, width=12)
plot(p_lines_mbte_zscore_stable_min3_stats)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg.byStudy.png", sep = ""), height=4.5, width=12, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats)
dev.off()
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_median.byStudy.pdf", sep = ""), height=4.5, width=12)
plot(p_lines_mbte_zscore_stable_min3_stat_median)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_median.byStudy.png", sep = ""), height=4.5, width=12, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stat_median)
dev.off()

p_lines_mbte_zscore_stable_min3_stat_median_newcolors <- 
  ggplot(data = patient_shared34_summary_filtered,
         aes(x = Study, y = mean, colour = Study, fill = Study, group = Study, shape = Tissue ), na.rm = T, se = TRUE) +
  scale_color_manual(values = trials_colors) +
  scale_fill_manual(values = trials_colors) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  # geom_point(alpha = 0.6, size = 4) +
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_mean.byStudy.colors.pdf", sep = ""), height=6, width=12)
plot(p_lines_mbte_zscore_stable_min3_stat_median_newcolors)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_mean.byStudy.colors.png", sep = ""), height=6, width=12, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stat_median_newcolors)
dev.off()
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg.byStudy.2cols.v2.pdf", sep = ""), height=7, width=6)
plot(p_lines_mbte_zscore_stable_min3_stats_2cols)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg.byStudy.2cols.v2.png", sep = ""), height=7, width=6, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats_2cols)
dev.off()
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.zoom_stats_avg.byLineage.pdf", sep = ""), height=5, width=14)
plot(p_lines_mbte_zscore_stable_min3_stats_lineage)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.zoom_stats_avg.byLineage.png", sep = ""), height=5, width=14, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats_lineage)
dev.off()

p_4studies_lines_mbte_zscore_stable_min3_stats_lineage <- 
  ggplot(data = patient_withSCID_shared34_summary_filtered,
         aes(x = CellLineage, y = mean, colour = CellMarker, fill = CellMarker, group = CellLineage, shape = Tissue), na.rm = T, se = TRUE) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  # geom_point(alpha = 0.6, size = 4) +
  geom_jitter(alpha = 0.6, size = 4) +
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.zoom_stats_avg.byLineage.age.v2.pdf", sep = ""), height=8, width=9)
plot(p_lines_mbte_zscore_stable_min3_stats_lineage_age)
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.zoom_stats_avg.byLineage.age.v2.png", sep = ""), height=8, width=9, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats_lineage_age)
dev.off()

p_lines_mbte_zscore_stable_min3_stats_lineage_age_median <- ggplot(data = patient_shared34_summary_filtered,
                                                                   # aes(x = CellLineage, y = median, colour = CellMarker, fill = CellMarker, group = CellLineage, shape = Tissue), na.rm = T, se = TRUE) +
                                                                   aes(x = CellLineage, y = mean, group = CellLineage, shape = Tissue), na.rm = T, se = TRUE) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  # geom_point(alpha = 0.6, size = 4) +
  geom_jitter(alpha = 0.6, size = 4) +
  facet_wrap(AgeGroup_Sorted ~ Study, nrow = 2) +
  ...
  theme(strip.text = element_text(face="bold", size=16)) +
  # theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") +
  theme(axis.text.x = element_text(size=16, angle = 20, vjust=1, hjust=1), 
        axis.text.y = element_text(size=16), 
        axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  # theme(strip.text.y = element_text(size = 16, colour = "blue", angle = 0)) +
  labs(title = paste0("Percentage of shared clones in CD34 cells"), 
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.zoom_stats_avg_median.byLineage.age.v2.pdf", sep = ""), height=8, width=8)
p_lines_mbte_zscore_stable_min3_stats_lineage_age_median
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.wrap_trials.zscore.zoom_stats_avg_median.byLineage.age.v2.png", sep = ""), height=8, width=8, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats_lineage_age_median)
dev.off()
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_median.byLineage.p1.v2.pdf", sep = ""), height=7, width=7)
p_lines_mbte_zscore_stable_min3_stats_age_median_p1
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_median.byLineage.p1.v2.png", sep = ""), height=7, width=7, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats_age_median_p1)
dev.off()
...
p_lines_mbte_zscore_stable_min3_stats_age_median_p2 <- 
  ggplot(data = patient_shared34_summary_filtered[which(patient_shared34_summary_filtered$Study == "BTHAL"),],
                                                              # aes(x = AgeGroup_Sorted, y = median, colour = CellMarker, fill = CellMarker, group = AgeGroup, shape = Tissue), na.rm = T, se = TRUE) +
                                                              aes(x = AgeGroup_Sorted, y = median, group = AgeGroup, shape = Tissue), na.rm = T, se = TRUE) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  # geom_point(alpha = 0.6, size = 4) +
  ...
  scale_y_continuous(limits = c(-2, 3)) +
  # stat_compare_means(comparisons = my_comparisons, label.y = seq(from = perc_starting_max_stats, to = (perc_starting_max_stats+(length(my_comparisons)*perc_spacer_stats)), by = perc_spacer_stats)*10^-1, p.adjust.method = "fdr", label = "p.signif") +
  stat_compare_means(comparisons = my_comparisons, label.y = seq(from = perc_starting_max_stats, to = (perc_starting_max_stats+(length(my_comparisons)*perc_spacer_stats)), by = perc_spacer_stats)*10^-1, label = "p.signif") +
  # stat_compare_means(comparisons = my_comparisons, label.y = c(2.5, 2), p.adjust.method = "fdr") +
  theme(strip.text = element_text(face="bold", size=16)) +
  # theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") +
  theme(axis.text.x = element_text(size=16, angle = 0), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
...
pdf(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_median.byLineage.p2.v2.pdf", sep = ""), height=7, width=3)
p_lines_mbte_zscore_stable_min3_stats_age_median_p2
dev.off()
png(file = paste0("analyses/cd34output/", analysis_folder_date, "/PaperRev1.", paste(study_list, collapse = "_"), ".CD34output.allpatients.v1.wrap_trials.zscore.zoom_stats_avg_median.byLineage.p2.v2.png", sep = ""), height=7, width=3, units = "in", res = 300)
plot(p_lines_mbte_zscore_stable_min3_stats_age_median_p2)
dev.off()
...
# p_value <- lapply(data_list, function(x) corrplot::cor.mtest(x[, 4:9], adjust="none", conf.level = 0.95, alpha=.05)[["p"]])
# p_value <- lapply(data_list, function(x) corrplot::cor.mtest(x[, 3:8], adjust="none", alpha=.05)[["p"]])
p_value <- lapply(data_list, function(x) corrplot::cor.mtest(x[, 1:6], adjust="none", alpha=.05)[["p"]])
p_value <- lapply(data_list, function(x) corrplot::cor.mtest(x[, 1:6], adjust="BH", alpha=.05)[["p"]])
full_correlations <- lapply(data_list, function(x) corrplot::cor.mtest(x[, 1:6], adjust="none", alpha=.05) )
correlation <- lapply(data_list, function(x) cor(x[, 1:6], method = "pearson"))
p_value_corrected_bonferroni <- as.data.frame(p_value) * 8 # since 15 are the combinations of tests
...
write.xlsx(x = p_value_corrected_bonferroni, file = paste("analyses/pca/", analysis_folder_date, "/RevPaper1.", analysis_folder_date, ".", ProjectID, ".my_data.colors.paperEdits.Pvals.Bonferroni.xlsx", sep = "") )
# -----------------------
...
pdf(file = paste("analyses/pca/", analysis_folder_date, "/RevPaper1.", analysis_folder_date, ".", ProjectID, ".Corrlations.t2.pdf", sep = ""), height=8, width=8)
correlations_t1 <- chart.Correlation(my_data[1:6], histogram=TRUE, pch=19, method = c("pearson"))
dev.off()
png(file = paste("analyses/pca/", analysis_folder_date, "/RevPaper1.", analysis_folder_date, ".", ProjectID, ".Corrlations.t2.png", sep = ""), height=8, width=8, units = "in", res = 300)
correlations_t1 <- chart.Correlation(my_data[1:6], histogram=TRUE, pch=19, method = c("pearson"))
dev.off()
...
names(my_data) <- c("Study", "TIGETID", "SubjectID", "H index", "HSPC size", "Tr.Efficiency", "VCN in vivo", "Age", "CD34 proKg", "CTC")
# res.pca <- prcomp(my_data[,4:9], scale = TRUE)
res.pca <- prcomp(my_data[,1:6], scale = TRUE)
...
# pca_contrib_dist
# dev.off()
pdf(file = paste("analyses/pca/", analysis_folder_date, "/", analysis_folder_date, ".", ProjectID, ".PCA.t2.newnames.RevPaper1.pdf", sep = ""), height=8, width=8)
pca_biplot_eig
pca_cos_dist
pca_contrib_dist
dev.off()
...
# pdf(file = paste("analyses/pca/", analysis_folder_date, "/", analysis_folder_date, ".", ProjectID, ".PCA.t2.groups.newnames.RevPaper1.pdf", sep = ""), height=6, width=7)
# pca_bp_gr2
# pca_bp_gr1
# dev.off()
pdf(file = paste("analyses/pca/", analysis_folder_date, "/", analysis_folder_date, ".", ProjectID, ".PCA.t2.groups.newnames.RevPaper2.pdf", sep = ""), height=6, width=7)
pca_bp_gr2
pca_bp_gr1
dev.off()
...
###############################################################
## Multi-to-Uni PLASTICITY COMMITMENT 
###############################################################
...
xscid_allpatients_flag_matrix_relabeled_correctedDC_freqs_melt_merge <- read.csv(file = paste0("/Users/calabria.andrea/Dropbox (HSR Global)/Clinical Trial MLD/analyses/XSCID_Commitment/202112/MyBTEry34_allMarkers_noLy_SC3/XSCID.AllPatients.hlfu_excl34MyBTEry.flag_matrix_relabeled_correctedDC_freqs_melt_merge.tsv"), header=TRUE, fill=T, sep='\t', check.names = FALSE, na.strings = c("NONE", "NA", "NULL", "NaN", ""))
...
## XSCID case
xscid_allpatients_flag_matrix_relabeled_correctedDC_freqs_melt_merge$ClassName <- xscid_allpatients_flag_matrix_relabeled_correctedDC_freqs_melt_merge$Class
xscid_profile_skewing_fulldata <- merge(x = xscid_allpatients_flag_matrix_relabeled_correctedDC_freqs_melt_merge, 
                                        y = color_schema_df[c("ClassName", "colorcode", "ColorSchema", "PlotLabel")], 
                                        by = c("ClassName"))
xscid_profile_skewing_fulldata$PlotLabel <- factor(xscid_profile_skewing_fulldata$PlotLabel, 
                                                   levels = lineage_faceting_order)
xscid_profile_skewing_fulldata_notNA <- xscid_profile_skewing_fulldata[which(!is.na(xscid_profile_skewing_fulldata$SonR)),]
xscid_profile_skewing_fulldata_notNA$CRonR <- ifelse((xscid_profile_skewing_fulldata_notNA$R > 0), 
                                                     xscid_profile_skewing_fulldata_notNA$CR *100 / xscid_profile_skewing_fulldata_notNA$R, 
                                                     NA)
xscid_profile_skewing_fulldata_notNA$Tissue <- "PB"
xscid_profile_skewing_fulldata_notNA <- xscid_profile_skewing_fulldata_notNA %>% group_by(ProjectID, SubjectID, Tissue, FollowUp) %>% mutate(CRonR_zscaled = scale(CRonR))
xscid_profile_skewing_fulldata_notNA[is.na(xscid_profile_skewing_fulldata_notNA)] <- NA
xscid_profile_skewing_fulldata_notNA$ClinicalStudy <- "XSCID"
...
##### XSCID test
scale_color_manual_colors_sortedbyname <- as.character(color_schema_df_sortedbyname[which(color_schema_df_sortedbyname$ClassName %in% levels(factor(xscid_profile_skewing_fulldata_notNA$Class))), "colorcode"])
scale_color_manual_colors_sortedbyplotlabel <- as.character(color_schema_df_sortedbyplotlabel[which(color_schema_df_sortedbyplotlabel$PlotLabel %in% levels(factor(xscid_profile_skewing_fulldata_notNA$PlotLabel))), "colorcode"])
# plot_CRonR_global_allFU <- ggplot(allstudies_profile_skewing_fulldata_notNA, aes(x = FollowUp, y = CRonR, fill = Class, color = Class), na.rm = T, se = TRUE) +
plot_CRonR_global_allFU_xscid <- ggplot(xscid_profile_skewing_fulldata_notNA, aes(x = FollowUp, y = CRonR, fill = PlotLabel, color = PlotLabel), na.rm = T, se = TRUE) +
  # geom_point(size = 2, alpha = 0.7) +
  # geom_smooth(stat = "smooth", position = "identity", span = 0.44) +
  # geom_smooth(method = "loess", formula = y ~ splines::ns(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  ...
  scale_fill_manual(values = scale_color_manual_colors_sortedbyplotlabel) +
  # scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(breaks = seq(0, max(xscid_profile_skewing_fulldata_notNA$FollowUp, na.rm = T), 12), limits = c(0, 60) ) +
  # facet_wrap( ~ ClinicalStudy, ncol = 3) +
  facet_grid(. ~ ClinicalStudy, scales = "free_x", space = "free_x") +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/", this_run_foldername, "XSCID.AllPatients.hlfu_34MyBT.maxFU60.pdf"), height=7, width=8)
print(plot_CRonR_global_allFU_xscid)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", this_run_foldername, "XSCID.AllPatients.hlfu_34MyBT.maxFU60.png"), height=7, width=8, units = "in", res = 300)
print(plot_CRonR_global_allFU_xscid)
dev.off()
#### end test
...
# global_patient_skewing_summary_filtered <- read.xlsx(xlsxFile = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.StableFU.CRonR.xlsx"))
...
##### ------------- aftre Good Turing application - Buscaroli Elena Caravagna [2023-10-11]
## ========= using global analysis and data --- re-apply stats just to use plot labels =========
# summary stats
excel_bagcases_foldername <- "/Users/calabria.andrea/Dropbox (HSR Global)/Clinical Trial MLD/analyses/13.hsc_profiles/"
excel_bagcases_filename <- "MLD.hlfu_34MyBTEry_noLy_no34exclusive.flag_matrix.bagcases.xlsx" # "MLD.hlfu_34MyBTEry.flag_matrix.bagcases.xlsx"  # "MLD.hlfu_34PBMyBT.flag_matrix.bagcases.xlsx" # MLD
...
global_patient_skewing_summary <- read.csv(file = "/Users/calabria.andrea/Dropbox (HSR Global)/TIGET/sharing/Giulio Caravagna/HSC dynamics/hsccommitment/datasets/PRED_commitment.csv", header=TRUE, fill=T, sep=',', check.names = FALSE, na.strings = c("NONE", "NA", "NULL", "NaN", ""))
# global_patient_skewing_summary_filtered <- global_patient_skewing_summary[which(global_patient_skewing_summary$fitname == "fit2"),]
global_patient_skewing_summary_filtered <- global_patient_skewing_summary[which(global_patient_skewing_summary$fitname == "fit1" & global_patient_skewing_summary$fittype == "noAge"),]
global_patient_skewing_summary_filtered <- merge(x = global_patient_skewing_summary_filtered, 
                                                 y = color_schema_df_sortedbyplotlabel[c("ClassName", "PlotLabel", "PaperOrder")],
                                                 by = c("ClassName"), all.x = T)
# global_patient_skewing_summary_filtered <- merge(x = global_patient_skewing_summary_filtered, 
#                                                  y = patients_summary[c("SubjectID", "Gender")],
#                                                  by = c("SubjectID"), all.x = T)
global_patient_skewing_summary_filtered$Study <- factor(global_patient_skewing_summary_filtered$ClinicalStudy, levels = study_list)

# scale_color_manual_colors_sortedbyname <- as.character(color_schema_df_sortedbyplotlabel[which(color_schema_df_sortedbyplotlabel$ClassName %in% levels(factor(global_patient_skewing_summary_filtered$Class))), "colorcode"])
scale_color_manual_colors_sortedbyname <- as.character(color_schema_df_sortedbyname[which(color_schema_df_sortedbyname$ClassName %in% levels(factor(global_patient_skewing_summary_filtered$ClassName))), "colorcode"])
# scale_color_manual_colors_sortedbyplotlabel <- as.character(color_schema_df_sortedbyplotlabel[which(color_schema_df_sortedbyplotlabel$PlotLabel %in% levels(factor(global_patient_skewing_summary_filtered$PlotLabel))), "colorcode"])
...
global_patient_skewing_summary_filtered <- global_patient_skewing_summary_filtered %>%
  mutate(AgeGroup = case_when(
    age <= 2 ~"0-2",
    age <= 15 ~"2-15",
    age > 15 ~"30+"
...
age_list <- c("0-2", "2-15", "30+")
global_patient_skewing_summary_filtered$AgeGroup_Sorted <- factor(global_patient_skewing_summary_filtered$AgeGroup, levels = age_list)
global_patient_skewing_summary_filtered$FollowUp <- global_patient_skewing_summary_filtered$Timepoint


global_patient_skewing_summary_filtered <- global_patient_skewing_summary_filtered %>% group_by(SubjectID, Tissue, FollowUp) %>% mutate(CRonR_zscaled = scale(pred_chao))
global_patient_skewing_summary_filtered[is.na(global_patient_skewing_summary_filtered)] <- NA

# slice data at stability
global_patient_skewing_summary_filtered_stable <- global_patient_skewing_summary_filtered[which(global_patient_skewing_summary_filtered$FollowUp >= 24 & global_patient_skewing_summary_filtered$FollowUp <= 60),]
global_patient_skewing_summary_filtered_stable <- global_patient_skewing_summary_filtered_stable %>% group_by(SubjectID, Tissue, ClassName) %>% mutate(n_obs_stt_overtime = n())
global_patient_skewing_summary_filtered_stable_min3 <- global_patient_skewing_summary_filtered_stable %>% group_by(SubjectID, Tissue, ClassName) %>% mutate(n_obs_stt_overtime = n()) %>% filter(n_obs_stt_overtime >= 3)


# summary stats
patient_skewing_summary <- global_patient_skewing_summary_filtered_stable %>%
  filter(n_obs_stt_overtime >= 2) %>%
  group_by(ClinicalStudy, SubjectID, Tissue, ClassName, age, AgeGroup, AgeGroup_Sorted, colorcode, PlotLabel, Gender) %>%
  get_summary_stats(CRonR_zscaled, type = "full") 
...
write.xlsx(x = patient_skewing_summary_filtered, file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.StableFU.CRonR.xlsx"))

plot_CRonR_allFU_nopoints_global_identicalFU <- ggplot(global_patient_skewing_summary_filtered, aes(x = FollowUp, y = pred_chao, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  # scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(breaks = seq(0, max(global_patient_skewing_summary_filtered$FollowUp, na.rm = T), 12), limits = c(0, 60)) +
  # facet_wrap( ~ ClinicalStudy, ncol = 3) +
  facet_grid(Tissue ~ Study, scales = "free_x", space = "free_x") +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.maxFU", followup_limit_crosstrial, ".v2.pdf"), height=8, width=10)
print(plot_CRonR_allFU_nopoints_global_identicalFU)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.maxFU", followup_limit_crosstrial, ".v2.png"), height=8, width=10, units = "in", res = 300)
print(plot_CRonR_allFU_nopoints_global_identicalFU)
dev.off()

plot_CRonR_allFU_nopoints_global <- 
  ggplot(global_patient_skewing_summary_filtered, aes(x = FollowUp, y = pred_chao, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  # scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(breaks = seq(0, max(global_patient_skewing_summary_filtered$FollowUp, na.rm = T), 12) ) +
  # facet_wrap( ~ ClinicalStudy, ncol = 3) +
  facet_grid(Tissue ~ Study, scales = "free_x", space = "free_x") +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.v2.pdf"), height=8, width=10)
print(plot_CRonR_allFU_nopoints_global)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.v2.png"), height=8, width=10, units = "in", res = 300)
print(plot_CRonR_allFU_nopoints_global)
dev.off()

plot_CRonR_allFU_nopoints_global_byage_PB <- 
  ggplot(global_patient_skewing_summary_filtered[which(global_patient_skewing_summary_filtered$Tissue == "PB"),], 
         aes(x = FollowUp, y = pred_chao, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname[-2]) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname[-2]) +
  # scale_y_continuous(limits = c(0, 100)) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.PB_byage.v2.pdf"), height=9, width=10)
print(plot_CRonR_allFU_nopoints_global_byage_PB)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.PB_byage.v2.png"), height=9, width=10, units = "in", res = 300)
print(plot_CRonR_allFU_nopoints_global_byage_PB)
dev.off()

plot_CRonR_allFU_nopoints_global_byage_PB_identicalFU <- 
  ggplot(global_patient_skewing_summary_filtered[which(global_patient_skewing_summary_filtered$Tissue == "PB"),], 
         aes(x = FollowUp, y = pred_chao, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname[-2]) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname[-2]) +
  # scale_y_continuous(limits = c(0, 100)) +
  scale_x_continuous(breaks = seq(0, max(global_patient_skewing_summary_filtered$FollowUp, na.rm = T), 12), limits = c(0, followup_limit_crosstrial)) +
  # facet_wrap( ~ ClinicalStudy, ncol = 3) +
  facet_grid(AgeGroup_Sorted ~ Study, scales = "free_x", space = "free_x") +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.PB_byage.maxFU", followup_limit_crosstrial, ".v2.pdf"), height=10, width=10)
print(plot_CRonR_allFU_nopoints_global_byage_PB_identicalFU)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.PB_byage.maxFU", followup_limit_crosstrial, ".v2.png"), height=10, width=10, units = "in", res = 300)
print(plot_CRonR_allFU_nopoints_global_byage_PB_identicalFU)
dev.off()

plot_CRonR_allFU_nopoints_global_byage_BM_identicalFU <- 
  ggplot(global_patient_skewing_summary_filtered[which(global_patient_skewing_summary_filtered$Tissue == "BM"),], 
         aes(x = FollowUp, y = pred_chao, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.BM_byage.maxFU", followup_limit_crosstrial, ".v2.pdf"), height=10, width=10)
print(plot_CRonR_allFU_nopoints_global_byage_BM_identicalFU)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR.nopoints.BM_byage.maxFU", followup_limit_crosstrial, ".v2.png"), height=10, width=10, units = "in", res = 300)
print(plot_CRonR_allFU_nopoints_global_byage_BM_identicalFU)
dev.off()

plot_CRonR_zscaled_global_allFU_nopoints_global <- 
  ggplot(global_patient_skewing_summary_filtered, 
         aes(x = FollowUp, y = CRonR_zscaled, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_x_continuous(breaks = seq(0, max(global_patient_skewing_summary_filtered$FollowUp, na.rm = T), 12) ) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR_zscaled.nopoints.pdf"), height=8, width=12)
print(plot_CRonR_zscaled_global_allFU_nopoints_global)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR_zscaled.nopoints.png"), height=8, width=12, units = "in", res = 300)
print(plot_CRonR_zscaled_global_allFU_nopoints_global)
dev.off()

plot_CRonR_zscaled_global_allFU_nopoints_byage <- 
  ggplot(global_patient_skewing_summary_filtered, 
         aes(x = FollowUp, y = CRonR_zscaled, fill = ClassName, color = ClassName), na.rm = T, se = TRUE) +
  geom_smooth(method = "loess", formula = y ~ log(x), stat = "smooth", position = "identity", alpha = 0.4, level = 0.75) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_x_continuous(breaks = seq(0, max(global_patient_skewing_summary_filtered$FollowUp, na.rm = T), 12) ) +
  facet_grid(Tissue + AgeGroup_Sorted ~ Study, scales = "free_x", space = "free_x") +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
  theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") +
  theme(axis.text.x = element_text(size=16), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR_zscaled.nopoints.byAge.pdf"), height=14, width=12)
print(plot_CRonR_zscaled_global_allFU_nopoints_byage)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.allFU.CRonR_zscaled.nopoints.byAge.png"), height=14, width=12, units = "in", res = 300)
print(plot_CRonR_zscaled_global_allFU_nopoints_byage)
dev.off()



write.xlsx(x = global_patient_skewing_summary_filtered, file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.StableFU.CRonR.xlsx"))
# global_patient_skewing_summary_filtered <- read.xlsx(xlsxFile = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.StableFU.CRonR.xlsx"))
global_patient_skewing_summary_filtered_bk <- global_patient_skewing_summary_filtered

scale_color_manual_colors_sortedbyname <- as.character(color_schema_df_sortedbyplotlabel[which(color_schema_df_sortedbyplotlabel$ClassName %in% levels(factor(patient_skewing_summary_filtered$ClassName))), "colorcode"])
# compare zscaled values
my_comparisons <- list(
  c("BTHAL", "WAS"),
  c("MLD", "BTHAL"),
...
perc_starting_max_stats <- 20
perc_spacer_stats <- 2.2
plot_stats_multiuni_zscaled_anova2 <- ggplot(data = patient_skewing_summary_filtered, aes(x = Study, y = median), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  geom_boxplot(alpha = .5, fill = "white", outlier.size = 0) +
  # geom_violin() +
  ...
  geom_jitter(aes(colour = PlotLabel), height = 0, width = 0.1, alpha = 0.7, size = 3) +
  facet_grid(Tissue ~ PlotLabel) +
  scale_y_continuous(limits = c(min(patient_skewing_summary_filtered$mean)*1.1, max(patient_skewing_summary_filtered$mean)*1.5)) +
  # geom_point(size=4, alpha = .7, position = "jitter") +
  # stat_compare_means(comparisons = my_comparisons, label.y = c(1, 1, 2, 2))+
  stat_compare_means(comparisons = my_comparisons, label.y = seq(from = perc_starting_max_stats, to = (perc_starting_max_stats+(length(my_comparisons)*perc_spacer_stats)), by = perc_spacer_stats)*10^-1, p.adjust.method = "fdr", label = "p.signif") +
  # stat_compare_means(comparisons = my_comparisons, label.y = seq(from = 2.5, to = 1.5, by = -0.5), label = "p.signif", p.adjust.method = "fdr") +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.CRonR.zscore.pdf"), height=8, width=11)
print(plot_stats_multiuni_zscaled_anova2)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.CRonR.zscore.png"), height=8, width=11, units = "in", res = 300)
print(plot_stats_multiuni_zscaled_anova2)
dev.off()

plot_stats_multiuni_zscaled_anova2_adolesc <- ggplot(data = patient_skewing_summary_filtered[which(patient_skewing_summary_filtered$AgeGroup == "2-15"),], 
                                             aes(x = Study, y = median), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  geom_boxplot(alpha = .5, fill = "white", outlier.size = 0) +
  # geom_violin() +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.age_2-15.CRonR.zscore.pdf"), height=8, width=11)
print(plot_stats_multiuni_zscaled_anova2_adolesc)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.age_2-15.CRonR.zscore.png"), height=8, width=11, units = "in", res = 300)
print(plot_stats_multiuni_zscaled_anova2_adolesc)
dev.off()
...
plot_stats_multiuni_zscaled_anova4_p1 <- ggplot(data = patient_skewing_summary_filtered, aes(x = AgeGroup_Sorted, y = median), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  geom_boxplot(alpha = .5, fill = "white", outlier.size = 0) +
  # geom_violin() +
...
plot_stats_multiuni_zscaled_anova4_p2 <- ggplot(data = patient_skewing_summary_filtered, aes(x = AgeGroup_Sorted, y = median), na.rm = T, se = TRUE) +
  scale_color_manual(values = scale_color_manual_colors_sortedbyname) +
  scale_fill_manual(values = scale_color_manual_colors_sortedbyname) +
  geom_boxplot(alpha = .5, fill = "white", outlier.size = 0) +
  # geom_violin() +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.CRonR.zscore.age-tissue.median.p1.v2.pdf"), height=16, width=8)
print(plot_stats_multiuni_zscaled_anova4_p1)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.CRonR.zscore.age-tissue.median.p1.v2.png"), height=16, width=8, units = "in", res = 300)
print(plot_stats_multiuni_zscaled_anova4_p1)
dev.off()
pdf(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.CRonR.zscore.age-tissue.median.p2.v2.pdf"), height=16, width=8)
print(plot_stats_multiuni_zscaled_anova4_p2)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/PaperRev1.", prefix_trials_outname, ".patient_skewing_summary_filtered.CRonR.zscore.age-tissue.median.p2.v2.png"), height=16, width=8, units = "in", res = 300)
print(plot_stats_multiuni_zscaled_anova4_p2)
dev.off()
...
###############################################################
## HSC COMMITMENT AT SINGLE CLONE - CELL FATE
###############################################################
faceting_order <- c("MLD", "WAS", "BTHAL")
lineage_faceting_order <- c("Multilineage", "Erythroid", "Myeloid", "B", "T")
trials_colors <- c("darkblue", "forestgreen", "firebrick")
...
# MLD.AllPatients.hlfu_excl34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv

# import all data
mld_hscf <- read.csv(file = paste0(mld_wd, "analyses/13.hsc_profiles/", analysis_folder_date, "/", this_run_foldername, "MLD.AllPatients.hlfu_excl34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv" ), header=T, fill=T, check.names = FALSE, sep = '\t', na.strings = c("NA", ''))
was_hscf <- read.csv(file = paste0(was_wd, "analyses/14.hsc_profiles/", analysis_folder_date, "/", this_run_foldername, "WAS.AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv" ), header=T, fill=T, check.names = FALSE, sep = '\t', na.strings = c("NA", ''))
bthal_hscf <- read.csv(file = paste0(bthal_wd, "analyses/14.hsc_profiles/", analysis_folder_date, "/", this_run_foldername, "BTHAL.AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv" ), header=T, fill=T, check.names = FALSE, sep = '\t', na.strings = c("NA", ''))

# merge
common_labels <- Reduce(intersect, list(colnames(mld_hscf), colnames(was_hscf), colnames(bthal_hscf)))
all_results_hscf_bm <- Reduce(rbind, list(mld_hscf[common_labels], was_hscf[common_labels], bthal_hscf[common_labels]))
all_results_hscf_bm$Tissue <- "BM"

# load PB too
this_run_foldername <- "MyBTEry34_allMarkers_noLy_PB_SC3/"
# import all data
mld_hscf <- read.csv(file = paste0(mld_wd, "analyses/13.hsc_profiles/", analysis_folder_date, "/", this_run_foldername, "MLD.AllPatients.hlfu_excl34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv" ), header=T, fill=T, check.names = FALSE, sep = '\t', na.strings = c("NA", ''))
was_hscf <- read.csv(file = paste0(was_wd, "analyses/14.hsc_profiles/", analysis_folder_date, "/", this_run_foldername, "WAS.AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv" ), header=T, fill=T, check.names = FALSE, sep = '\t', na.strings = c("NA", ''))
bthal_hscf <- read.csv(file = paste0(bthal_wd, "analyses/14.hsc_profiles/", analysis_folder_date, "/", this_run_foldername, "BTHAL.AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.csv" ), header=T, fill=T, check.names = FALSE, sep = '\t', na.strings = c("NA", ''))
# merge
common_labels <- Reduce(intersect, list(colnames(mld_hscf), colnames(was_hscf), colnames(bthal_hscf)))
all_results_hscf_pb <- Reduce(rbind, list(mld_hscf[common_labels], was_hscf[common_labels], bthal_hscf[common_labels]))
all_results_hscf_pb$Tissue <- "PB"

# merge all data of the tissues
all_results_hscf <- rbind(all_results_hscf_bm, all_results_hscf_pb)
all_results_hscf$Study <- factor(all_results_hscf$ProjectID, levels = study_list)
all_results_hscf[is.na(all_results_hscf)] <- NA

# add labels
all_results_hscf$Label <- apply(X = all_results_hscf[c("Early_StateLabel","Late_StateLabel")], 1, paste, collapse = '->')
# compare data (now importing z-scaled fitted values)
all_results_hscf <- all_results_hscf %>% group_by(SubjectID, Tissue) %>% mutate(zscaled_perc_PersistentClones=scale(perc_PeristentClones))
all_results_hscf_patstoremove <- all_results_hscf %>% group_by(SubjectID, Tissue) %>% summarise(n_labels = n())
# MLD11 MLD20 MLD05 MLD08 BTHAL003
patients_to_exclude <- c("MLD11", "MLD20", "MLD05", "MLD08", "BTHAL003")
all_results_hscf$to_exclude <- ifelse(all_results_hscf$SubjectID %in% patients_to_exclude, T, F)
# labeling factor for plotting
all_results_hscf$StateLabel_Class <- ifelse(all_results_hscf$Early_StateLabel == "Multi", "Multi", "Uni")

# save data
write.xlsx(x = all_results_hscf, file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.fulldata.xlsx"))
# all_results_hscf <- read.xlsx(xlsxFile = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.fulldata.xlsx"))
all_results_hscf$Study <- factor(all_results_hscf$Study, levels = faceting_order)
...
# summary stats
all_results_hscf_summary <- all_results_hscf[which(!(all_results_hscf$SubjectID %in% patients_to_exclude)),] %>%
  group_by(Study, Label, Tissue, StateLabel_Class) %>%
  # get_summary_stats(PercNIS_onOverallSharedCD34BM_zscaled, type = "mean_sd")
  get_summary_stats(perc_PeristentClones, type = "full")

write.xlsx(x = all_results_hscf_summary, file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.xlsx"))

all_results_hscf_filtered <- all_results_hscf[which(!(all_results_hscf$SubjectID %in% patients_to_exclude)),]
all_results_hscf_filtered$Study <- factor(all_results_hscf_filtered$Study, levels = faceting_order)
...
# manual label colors
# Multi->Multi	green4
# Multi->UniErythroid	red1
# Multi->UniLyB	dodgerblue
# Multi->UniLyT	deepskyblue2
# Multi->UniMyeloid	goldenrod1 
# UniErythroid->UniErythroid	red3
# UniLyB->UniLyB	dodgerblue3
# UniLyT->UniLyT	deepskyblue
# UniMyeloid->UniMyeloid	orange
labels_colors_manual <- c("green4", "red1", "dodgerblue", "deepskyblue2", "goldenrod1 ", "red3", "dodgerblue3", "deepskyblue", "orange")
# try plots
plot_all_results_hscf_study_stackedbars <- ggplot(all_results_hscf_summary, aes(fill=Label, y=mean, x=Study)) + 
  scale_color_manual(values = labels_colors_manual) +
  scale_fill_manual(values = labels_colors_manual) +
  geom_bar(position="stack", stat="identity") +
  # geom_errorbar( aes(ymin=mean-se, ymax=mean+se), width=0.4, colour="black", alpha=0.9, size=1, position="identity") +
  # facet_wrap( Tissue ~ ., nrow = 1) +
  facet_grid( Tissue ~ StateLabel_Class) +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
  theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") + 
  theme(axis.text.x = element_text(size=16), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  # theme(strip.text.y = element_text(size = 16, colour = "blue", angle = 0)) +
  labs(title = paste0("HSPC commitment - multi-uni lineage at single clone"), subtitle = paste0("Ealry vs Late time points, threshold 24 months. Only persistent clones analyzed (present both early and late)."),
       x = "Study", 
       y = "Commitment class percentage", 
       colour = "Label", fill = "Label")

pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.pdf"), height=8, width=10)
plot(plot_all_results_hscf_study_stackedbars)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.png"), height=8, width=10, units = "in", res = 300)
plot(plot_all_results_hscf_study_stackedbars)
dev.off()

plot_all_results_hscf_study_stackedbars_horiz <- ggplot(all_results_hscf_summary, aes(fill=Label, y=mean, x=Study)) + 
  scale_color_manual(values = labels_colors_manual) +
  scale_fill_manual(values = labels_colors_manual) +
  geom_bar(position="stack", stat="identity") +
  # geom_errorbar( aes(ymin=mean-se, ymax=mean+se), width=0.4, colour="black", alpha=0.9, size=1, position="identity") +
  # facet_wrap( Tissue ~ ., nrow = 1) +
  facet_grid( StateLabel_Class ~ Tissue) +
  scale_y_continuous(limits = c(0,100)) +
  theme_bw() +
  theme(strip.text = element_text(face="bold", size=16)) +
  theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") + 
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.horizontal.pdf"), height=8, width=10)
plot(plot_all_results_hscf_study_stackedbars_horiz)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.summary.horizontal.png"), height=8, width=10, units = "in", res = 300)
plot(plot_all_results_hscf_study_stackedbars_horiz)
dev.off()

n_fun <- function(x){
  return(data.frame(y = c(min(all_results_hscf_filtered$zscaled_perc_PersistentClones)*1.8),
                    label = paste0("N=", length(x)) ))
}
# stat_summary(fun.data = n_fun, geom = "text", hjust = 0.5, size = 5) +
plot_all_results_hscf_study_allpoints <- 
  ggplot(all_results_hscf_filtered, aes(x=Label, y=zscaled_perc_PersistentClones, fill=Label, colour = Label, group = Label), na.rm = T, se = TRUE) + 
  scale_color_manual(values = labels_colors_manual) +
  scale_fill_manual(values = labels_colors_manual) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
  ...
  theme(strip.text = element_text(face="bold", size=16)) +
  theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") + 
  theme(axis.text.x = element_text(size=16, angle = 90, vjust = 0.5, hjust=1), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  # theme(strip.text.y = element_text(size = 16, colour = "blue", angle = 0)) +
  labs(title = paste0("HSPC commitment - multi-uni lineage at single clone"), subtitle = paste0("Ealry vs Late time points, threshold 24 months. Only persistent clones analyzed (present both early and late).\nPercentages normalized by Z-score."),
       x = "Label", 
       y = "Commitment class percentage", 
       colour = "Label", fill = "Label")

pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.pdf"), height=10, width=14)
plot(plot_all_results_hscf_study_allpoints)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.png"), height=10, width=14, units = "in", res = 300)
plot(plot_all_results_hscf_study_allpoints)
dev.off()

# find the combination of all comparisons from labels
label_comparisons <- as.data.frame(combn(levels(factor(all_results_hscf$Label)), m=2))
...
n_fun <- function(x){
  return(data.frame(y = c(min(all_results_hscf$zscaled_perc_PersistentClones)*1.1),
                    label = paste0("N=", length(x)) ))
}
# stat_summary(fun.data = n_fun, geom = "text", hjust = 0.5, size = 5) +

plot_all_results_hscf_study_allpoints_statsbystudy <- 
  ggplot(all_results_hscf_filtered, aes(x=Study, y=zscaled_perc_PersistentClones, fill=Study, colour = Study, group = Study), na.rm = T, se = TRUE) + 
  scale_color_manual(values = trials_colors) +
  scale_fill_manual(values = trials_colors) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
  facet_grid(Tissue ~ Label) +
  scale_y_continuous(limits = c(-1.5, 4), breaks = c(-1, seq(0, 4))) + 
  stat_summary(fun.data = n_fun, geom = "text", hjust = 0.5, size = 5) +
  theme_bw() +
  stat_compare_means(comparisons = my_comparisons, label.y = seq(from = perc_starting_max_stats, to = (perc_starting_max_stats+(length(my_comparisons)*perc_spacer_stats)), by = perc_spacer_stats)*10^-1, p.adjust.method = "fdr", label = "p.signif") +
  ...
  labs(title = paste0("HSPC commitment - multi-uni lineage at single clone"), subtitle = paste0("Ealry vs Late time points, threshold 24 months. Only persistent clones analyzed (present both early and late).\nPercentages normalized by Z-score."),
       x = "Studies", 
       y = "HSC commitment (Z-scaled percentage)", 
       colour = "Study", fill = "Study")

pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.pdf"), height=8, width=20)
plot(plot_all_results_hscf_study_allpoints_statsbystudy)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.png"), height=8, width=20, units = "in", res = 300)
plot(plot_all_results_hscf_study_allpoints_statsbystudy)
dev.off()
...
# lineage colors
perc_spacer_stats <- -3
perc_starting_max_stats <- 30
n_fun <- function(x){
  return(data.frame(y = c(min(all_results_hscf_filtered$zscaled_perc_PersistentClones)*1.8),
                    label = paste0("N=", length(x)) ))
}
plot_all_results_hscf_study_stats <- 
  ggplot(all_results_hscf_filtered, aes(x=Study, y=zscaled_perc_PersistentClones, fill=Label, colour = Label, group = Study), na.rm = T, se = TRUE) + 
  scale_color_manual(values = labels_colors_manual) +
  scale_fill_manual(values = labels_colors_manual) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
  facet_grid( Tissue ~ Label, scales = "free_x", space = "free") +
  stat_summary(fun.data = n_fun, geom = "text", hjust = 0.5, size = 5) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.linCols.pdf"), height=8, width=20)
plot(plot_all_results_hscf_study_stats)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.linCols.png"), height=8, width=20, units = "in", res = 300)
plot(plot_all_results_hscf_study_stats)
dev.off()

# now 2 plots (multi and uni)
plot_all_results_hscf_study_stats_multi <- 
  ggplot(all_results_hscf_filtered[which(all_results_hscf_filtered$StateLabel_Class == "Multi"),], aes(x=Study, y=zscaled_perc_PersistentClones, fill=Label, colour = Label, group = Study), na.rm = T, se = TRUE) + 
  scale_color_manual(values = labels_colors_manual) +
  scale_fill_manual(values = labels_colors_manual) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
  ...
  stat_compare_means(comparisons = my_comparisons, 
                     label.y = seq(from = perc_starting_max_stats, to = (perc_starting_max_stats+(length(my_comparisons)*perc_spacer_stats)), 
  ...
  theme(strip.text = element_text(face="bold", size=10)) +
  theme(legend.direction = "horizontal", legend.position = "bottom", legend.box = "horizontal") + 
  theme(axis.text.x = element_text(size=16, angle = 30, vjust = 1, hjust=1), axis.text.y = element_text(size=16), axis.title = element_text(size=16), plot.title = element_text(size=20)) +
  # theme(strip.text.y = element_text(size = 16, colour = "blue", angle = 0)) +
  labs(title = paste0("HSPC commitment - multi-uni lineage at single clone"), subtitle = paste0("Ealry vs Late time points, threshold 24 months. Only persistent clones analyzed (present both early and late).\nPercentages normalized by Z-score."),
       x = "Label", 
       y = "Commitment class percentage", 
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.linCols.MULTI.pdf"), height=8, width=11)
plot(plot_all_results_hscf_study_stats_multi)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.linCols.MULTI.png"), height=8, width=11, units = "in", res = 300)
plot(plot_all_results_hscf_study_stats_multi)
dev.off()

plot_all_results_hscf_study_stats_uni <- 
  ggplot(all_results_hscf_filtered[which(all_results_hscf_filtered$StateLabel_Class == "Uni"),], aes(x=Study, y=zscaled_perc_PersistentClones, fill=Label, colour = Label, group = Study), na.rm = T, se = TRUE) + 
  scale_color_manual(values = labels_colors_manual[6:length(labels_colors_manual)]) +
  scale_fill_manual(values = labels_colors_manual[6:length(labels_colors_manual)]) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
  facet_grid( Tissue ~ Label, scales = "free_x", space = "free") +
  stat_summary(fun.data = n_fun, geom = "text", hjust = 0.5, size = 5) +
...
pdf(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.linCols.UNI.pdf"), height=8, width=8.8)
plot(plot_all_results_hscf_study_stats_uni)
dev.off()
png(file = paste0("analyses/multi_uni_sharing/202112/", paste(study_list, collapse = "_"), ".AllPatients.hlfu_34MyBTEry.flag_matrix_timeShared_relabeled_singleclone.full_zscale.stat_study.linCols.UNI.png"), height=8, width=8.8, units = "in", res = 300)
plot(plot_all_results_hscf_study_stats_uni)
dev.off()
...
# on percentage
perc_spacer_stats <- -50
perc_starting_max_stats <- 700
n_fun <- function(x){
  return(data.frame(y = c(min(all_results_hscf$perc_PeristentClones)*1.1),
                    label = paste0("N=", length(x)) ))
}
plot_all_results_hscf_study_allpoints_statsbystudy_perc <- 
  ggplot(all_results_hscf_filtered, aes(x=Study, y=perc_PeristentClones, fill=Study, colour = Study, group = Study), na.rm = T, se = TRUE) + 
  scale_color_manual(values = trials_colors) +
  scale_fill_manual(values = trials_colors) +
  geom_boxplot(colour = "lightblue", fill = "lightblue", alpha = .3, outlier.size = 0) +
  geom_jitter(alpha = 0.6, size = 4) +
